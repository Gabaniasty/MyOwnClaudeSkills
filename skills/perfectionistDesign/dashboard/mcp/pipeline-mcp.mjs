...
const TOOLS = [
  ...
  { name: "generate_mockup",
    description: "PHASE 1, ALWAYS FIRST on any new page or redesign. GATE 39: generate AT LEAST TWO different mockups and let the USER choose - never one, and never pick for them. Write scratch/prompts/_mockup_a.txt and _mockup_b.txt (add _mockup_c when the layout carries money, there are 8+ sections, you have only a feeling and no reference, or you are unsure - unsure means go UP, never down to one), each describing the WHOLE page top to bottom, every section in order. Then call this once; it renders every _mockup*.txt it finds, or just the slugs you pass in `only`. The variants must offer genuinely DIFFERENT DEVICES - different ideas, not three dressings of one, and never different genres. Hold the section list, copy and palette identical so the choice is about the idea. Never let a genre word (editorial, Swiss, brutalist, punk, minimal, premium-consumer) appear in a label the user reads. When it finishes, READ each images/_masters/_mockup_*.png with the Read tool, SHOW them all to the user with one line each on what it does well and what it costs, give a recommendation labelled as a recommendation, ASK WHICH TO BUILD, AND STOP. Only after they choose do you extract the design system from the chosen one. Generate a single mockup ONLY when the user supplied a reference image of the NEW design or explicitly asked for one.",
